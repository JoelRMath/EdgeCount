...
#'
#' @description Finds all pairs of terms from an \code{ECTermScoring} object that have at least one edge
#' connecting their *disjoint* parts in the interaction network (\code{ECProb}).
#' This version uses fast anti-joins on the observed edge list to filter out connections
...
#' @param ects An \code{ECTermScoring} object (the bipartite graph).
#'
#' @return A \code{data.table} containing the pairs ("set1", "set2") and their
#'   disjoint \code{observed_edges} count. Only pairs with at least one disjoint edge are returned.
#' @export
setGeneric("get_disjoint_connected_pairs", function(object, ects) standardGeneric("get_disjoint_connected_pairs"))

#' @describeIn get_disjoint_connected_pairs Method for ECProb and ECTermScoring objects.
setMethod("get_disjoint_connected_pairs", signature(object = "ECProb", ects = "ECTermScoring"),
          function(object, ects) {
