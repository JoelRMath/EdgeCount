...
\end{itemize}
Observed values $z$ are obviously dependent of set sizes, but they should also be made conditional to degrees of the vertices in the set(s). Hence, to an observed value $z$ is associated a corresponding random variable $Z$ in the RGGED.
...
For a within variable associated to a set $S$ we have
\begin{equation}
m = \left| S \right| \left(\left| S \right| -1 \right)/2 \quad \text{and} \quad \lambda = \sum_{i < j, \in S} p_{ij}.
\end{equation}
For a between variable associated to two sets $S_1$ and $S_2$ we have
\begin{equation}
...
The addition of 3/8 stems from the work of \cite{anscombe1948}, who showed that $\sqrt{X + 3/8}$ is the optimal variance-stabilizing transformation for Poisson data. The $lAr$ metric is effectively the log2-ratio of these variance-stabilized values.
