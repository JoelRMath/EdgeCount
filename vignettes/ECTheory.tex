...
Now, approximate $p_{ij}$ values with $k_ik_j/(2M)$ is a conservative approach with respect to the RGGED. Namely, using this approximation
\begin{itemize}
\item[-] overestimates $p_ij$ values,
...
The approximation is called {\bf fast approximation} because it enables faster calculation of edge-count statistics.
...
\end{equation}
A naive implementation would have time complexity $\mathcal{O} \left( n^2 \right)$. Now, recall the expression for the square of a sum
\begin{equation}
\left( \sum_{i \in S} k_i \right)^2 = \sum_{i \in S} k_i^2 + 2 \sum_{i < j} k_i k_j.
\end{equation}
Rearranging the cross terms yields
\begin{equation}
